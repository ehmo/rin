# Architecture and Technical Design

## Scope
Translate product behavior and contracts into service boundaries and implementation-ready architecture.

## Focus Areas
- Domain ownership and state transitions.
- Command/query API contracts.
- Event envelope and subject taxonomy.
- Data boundary rules and migration guardrails.

## Working Docs
- `docs/architecture/SYSTEM_ARCHITECTURE_DB_FIRST_V2.md`
- `docs/architecture/SERVICE_IMPLEMENTATION_BLUEPRINT_V1.md`
- `docs/architecture/IOS_APP_ARCHITECTURE_V1.md`
- `docs/architecture/SECRETS_CONFIG_MANAGEMENT_V1.md`
- `docs/architecture/IOS_NAVIGATION_STATE_V1.md`
- `docs/architecture/IOS_API_CLIENT_V1.md`
- `docs/architecture/IOS_OFFLINE_STORAGE_V1.md`
- `docs/architecture/BACKEND_MIGRATION_GUIDE_V1.md`
- `docs/architecture/SEARCH_RELEVANCE_STRATEGY_V1.md`
- `docs/architecture/RANKING_QUALITY_EVALUATION_V1.md`
- `docs/architecture/DATA_RETENTION_DELETION_V1.md`
- `docs/architecture/EXPERIMENTATION_FRAMEWORK_V1.md`
- `docs/architecture/DATA_MODEL_BOUNDARIES_V1.md`
- `docs/architecture/OWNERSHIP_STATE_MACHINE_SPEC_V1.md`
- `docs/architecture/EVENT_CONTRACT_CATALOG_V1.md`
- `docs/architecture/MONOREPO_CONVENTIONS_V1.md`
- `docs/architecture/DEVELOPER_PLATFORM_VISION_V1.md`
- `docs/architecture/API_PRODUCTIZATION_TIERS_V1.md`
- `docs/architecture/KYC_TRUST_ASSERTIONS_V1.md`
- `docs/architecture/ONPREM_HYBRID_ARCHITECTURE_V1.md`

## Beads Mapping
- `rin-3i0.4.*`
- `rin-3i0.1.*`
- `rin-3i0.18.*`

Prev: [Design System and Interaction Language](03-design-system.md)
Next: [Data, Search, and Ranking](05-data-search-ranking.md)
